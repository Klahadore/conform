        ...
        [
            {
                ...
                "content": [
                 ...
                 - Include the CDN for Tailwind CSS: `<script src="https://cdn.tailwindcss.com"></script>`
                 - The progress bar must be implemented as a fixed div at the top with id "progressBar"
                 - Include combobox functionality for dropdown fields
                 - Include tooltips for help text
                 - The JavaScript must handle:
                 - Step navigation (nextStep/prevStep functions)
...
def chain1(pdf_path, field_mapping_string, hospital_system=None):
    ...
    try:
        ...
        db_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "conform.db"
        )
        ...
        relative_html_path = (
            os.path.join(safe_hospital_system, html_filename)
            if hospital_system
            else html_filename
        )
